...
# Repository permissions: read:Contents, read:Metadata, read:Commit statuses
HEADERS = {'authorization': 'token ' + os.environ['ACCESS_TOKEN']}
USER_NAME = os.environ['USER_NAME']  # 'Code-Ph0enix'
QUERY_COUNT = {'user_getter': 0, 'follower_getter': 0, 'graph_repos_stars': 0,
               'recursive_loc': 0, 'graph_commits': 0, 'loc_query': 0}


def daily_readme(birthday):
    """Returns age string e.g. '22 years, 3 months, 23 days'"""
...
def graph_commits(start_date, end_date):
    """Total commit count via GraphQL contributionsCollection"""
...
def graph_repos_stars(count_type, owner_affiliation, cursor=None):
    """Repos or stars count via GraphQL"""
...
def recursive_loc(owner, repo_name, data, cache_comment,
    ...
    """Fetch lines of code from a repo via cursor pagination (100 commits at a time)"""
...
def loc_query(owner_affiliation, comment_size=0, force_cache=False, cursor=None, edges=[]):
    """Queries all repos and returns total lines of code"""
...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data,
    ...
    """Parse SVG and update dynamic stat elements"""
...
if __name__ == '__main__':
    ...
    # DOB: 18 March 2004
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2004, 3, 18))
    ...
    follower_data, fol_time  = perf_counter(follower_getter, USER_NAME)

    formatter('commits',   commit_time)
    formatter('stars',     star_time)
    formatter('repos',     repo_time)
    formatter('followers', fol_time)
    ...
    for name, count in QUERY_COUNT.items():
        print('{:<28}'.format('   ' + name + ':'), '{:>6}'.format(count))
